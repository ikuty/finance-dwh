...
低スペックの計算機（Mac Mini 2012、7.7GBメモリ、HDDストレージ）とスマートプラグによる電源枠管理を
...
Mac Mini（Ubuntu 24.04、7.7GB RAM）上でsystemdタイマーにより日次実行。スマートプラグ
...
- **Mac Miniのメモリ制約（7.7GB）**: DuckDBの`memory_limit`を明示的に制限している
  （実機でのメモリ逼迫・スワップ発生を踏まえた対策）。大量データの一括処理（初回
