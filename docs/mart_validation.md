...
## 既知の外れ値（調査終了、2026-09-21）

intermediate層導入（会計基準ごとのモデル分離）により、`sales`単調性違反は24件→9件まで
減少した（日本ハム(2282)・CLホールディングス(4286)は会計基準またぎのcoalesce誤りが
根本原因で、intermediate層導入により解消）。残り7件（6社）は、element_idレベルまで
確認した結果、当システム側の抽出ロジックの問題ではなく、**annual書類側のXBRL値自体が
四半期報告書と整合しない特異点**と判断した（例: 三井海洋開発(6269)38期は、annual・
q3とも同一XBRL要素`jpcrp_cor:RevenueIFRSSummaryOfBusinessResults`を使っているにも
関わらず、annual側の値だけが四半期の1/100程度に小さい。提出者側のデータ特性の
可能性が高く、当システムの責務範囲外と判断）。

対象: クシム(2345)28期、アステリア(3853)26期、マクロミル(3978)10期、
アドベンチャー(6030)16期、三井海洋開発(6269)38期・40期、
アジア開発キャピタル(9318)103期。
