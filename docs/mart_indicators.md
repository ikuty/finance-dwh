...
| `eps` | 1株当たり当期純利益（実績、会社予想値ではない） | 円/株(JPYPerShares) | １株当たり当期純利益又は当期純損失（△） | 基本的１株当たり利益又は損失（△）（IFRS） | 基本的１株当たり当社株主に帰属する利益又は損失（△）（US GAAP） |
...
| `per` | 株価収益率（企業自己申告、実績EPSベース。詳細は後述） | 無単位(pure) | 株価収益率 | 株価収益率（IFRS） | 株価収益率（US GAAP） |
...
`eps`（延いては`per`・`earnings_yield`）はEDINET開示（有価証券報告書・四半期報告書）に
基づく**確定済み実績値**であり、証券会社サイト等で一般的にデフォルト表示される
「予想PER」（会社が発表した進行期の業績予想EPSを使う）とは異なる。

実機検証（2026-09-25、3087 ドトール・日レスホールディングス、株価3,210円時点）で各サイトの
表示を突合した。

| サイト | 表示PER | 使用EPS（逆算） | 実際の該当値 | 基準 |
|---|---:|---:|---:|---|
| かぶたん | 17.9倍 | 179.3円 | 179.6円（会社予想、進行期2027.02期） | 予想EPS |
| Yahoo!ファイナンス | 17.85倍 | 179.8円 | 179.6円（会社予想、UI上に「会社予想」と明記） | 予想EPS |
| みんかぶ | 18.8倍 | 170.7円 | 170.7円（直近確定期2026.02期の実績） | **実績EPS** |

みんかぶの実績PER表示と一致（3210÷170.7≒18.81）。かぶたん・Yahoo!ファイナンスと比較する
場合は、この基準の違いを踏まえる必要がある。会社予想EPSは決算短信・適時開示の「業績予想」
欄に記載される情報で、EDINETの有価証券報告書・四半期報告書には通常含まれないため、現状の
データソースからは予想PERを算出できない（別データソースが必要、未対応）。
