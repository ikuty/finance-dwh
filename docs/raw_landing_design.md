...
### 重要な落とし穴: OHLC列はx0ではなくx1（右端）で判定する必要がある
...
（`jpx_monthly_ohlc_facts.py`の`_column_for_word()`参照）。
...
### 実機検証（2025-09分、1ヶ月、修正後）

| 検証内容 | 結果 |
|---|---|
| レコード数 | 88,184件（20営業日） |
| 重複(code, file_date) | 0件 |
| OHLC整合性（前場・後場それぞれ 安値<=始値・終値<=高値） | 167,095件検証、違反0件（x1ベース修正後） |
| 抽出時間 | 約62秒（1,225ページ、ローカルMac実測） |
